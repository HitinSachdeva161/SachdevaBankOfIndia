...
class srchnm: 
    def __init__(self):
        ...
        self.root.title("Search By Name")
        self.root.config(background='#51b9fa')
        self.l1=Label(self.root,text='Search By Name',font=("Arial", 30,'bold'))
        self.l1.pack(pady=5)
        self.l2=Label(self.root,text="Enter Name-",background='#51b9fa',font=("Century Schoolbook", 10,'bold'))
    ...
    def qry(self):
        ...
        cursor=conn.cursor()
        cursor.execute("select * from tblaccount where cname=%s",(val))
        conn.commit()
