...
class withd: 
    ...
    def __init__(self):
        ...
        self.ll2=Label(self.root,text="Withdraw",background='white',font=("Century Schoolbook", 15,'bold'))
        img=Image.open('E:\eclipse\work_eclipse\Bank_project\images\withdraw.jpg')
        i2=img.resize((40,40))
        ...
        self.l2=Label(self.root,text="Amount to be Withrawn",background='#83cdfb')
        self.t1=Entry(self.root)
        self.t2=Entry(self.root)
        self.b1=Button(self.root,text="Withdraw",command=self.qry)
    ...
    def qry(self):
        ...
        if self.t1.get()=="" or self.t2.get()=="" or int(self.t2.get())<0:
            ...
            #====================================================================
                ...
                cursor.execute("INSERT INTO tbltransaction SET transid=%s,transsrcaccno=%s,transtype='Withdraw',transamt=%s,transdate=%s,transtime=%s",(id,acc,dep,d1,crt))
                conn2.commit()
                conn2.close()
                messagebox.showinfo("Successful","Transaction Completed")
            
            else:
                messagebox.showerror("Error","No A/c Found / Low Balance")
    def bk(self):
        self.root.destroy()
        from home_pg import Mn
        op=Mn()        
